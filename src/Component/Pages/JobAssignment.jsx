 ...
 console.log(selectedEmployee,"selected emp")
  useEffect(() => {
    ...
    const newEvents = selectedEmployee.assignedJobs.flatMap((job) =>
        job.customerAvailability.map((availability) => {
  ...
  console.log("Selected Employee ID:", selectedEmployee?._id, "Employee Events:", employeeEvents);
  return (
  ...
  <div 
    ...
    className={`p-2 border rounded cursor-pointer mb-2 ${
      selectedEmployee?._id === employee._id ? 'bg-green-400' : 'bg-green-200'
    }`}
  > 
    {employee.name}
  </div>
  ...
  .map((ev) => (
    <div key={ev.id} className="mt-1 p-1 bg-blue-400 text-white rounded shadow text-xs font-semibold">
      ...
      <div className="w-1/4 h-fit p-4 border rounded bg-gray-100">
